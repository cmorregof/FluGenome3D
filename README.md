...
It grew out of my AntigenLM/AntigenSDE thesis work as a satellite project: the Python pipeline audits sequence context, codon/CDS behavior, simple representations and deterministic tokenization; the web app turns only safe derived artifacts into an interface that can be shared selectively.
...
- A governance-first app that separates restricted local analysis from safe public exports.
...
Vercel safe mode uses only `app/data/*.safe.json`, generated by the safe export script from real derived artifacts.
